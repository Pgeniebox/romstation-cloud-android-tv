...
public class WebRtcBridge {
    ...
    private final NativePcmPlayer pcmPlayer;
    ...
    private final Runnable keepAliveRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (keepAliveIp != null) {
                headers.put("X-Forwarded-For", keepAliveIp);
            }
            ...
            request.startRequestNetwork(RequestNetworkController.POST, keepAliveUrl, "keepalive", new RequestNetwork.RequestListener() {
                ...
                public void onErrorResponse(String tag, String message) {
                    ...
                    // Fast retry on error (10s) to ensure initial success
    ...
    public WebRtcBridge(MainActivity activity, NativePcmPlayer pcmPlayer) {
    ...
    public void setAudioSampleRate(int rate) {
        Log.w(TAG, "pcmPlayer=" + (pcmPlayer == null ? " null" : " exist"));
        if (pcmPlayer != null) {
            pcmPlayer.setSampleRate(rate);
        }
    }

    @JavascriptInterface
    public void sendAudioFrame(byte[] audioFrame) {
        if (pcmPlayer != null && audioFrame != null && audioFrame.length > 0) {
            //Log.w(TAG, " pcmPlayer.play(audioFrame):" + audioFrame.length);

            pcmPlayer.play(audioFrame);
        } else if (audioFrame == null || audioFrame.length == 0) {
            //Log.w(TAG, "Received empty or null audio frame from JS");
        }
    }
    ...
    public void closeWebview() {
        activity.runOnUiThread(() -> {
            if (activity.webView != null)  activity.closeWebview();
        });
    }
    @JavascriptInterface
    ...
    @JavascriptInterface
    public void keepalive(String url, String token, String ip) {
        this.keepAliveUrl = url;
        this.keepAliveToken = token;
        this.keepAliveIp = ip;
        this.firstKeepAliveSuccess = false;
        keepAliveHandler.removeCallbacks(keepAliveRunnable);
        keepAliveHandler.postDelayed(keepAliveRunnable, 15000); // Start after 15 seconds
    }
}
