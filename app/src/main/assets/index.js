                                                            ...
                                                            if(CXtoken.length&&CXtoken.length>0&&CXsessionPath.length&&CXsessionPath.length>0&&CXendPoint.length&&CXendPoint.length>0)
                                                           {
                                                            ...
                                                            console.log("[BxC] Keepalive URL:", CXurl,' token: ',CXtoken);
                                                             if (window.AndroidBridge && window.AndroidBridge.keepalive) {
                                                                            ...
                                                                            window.AndroidBridge.keepalive(CXurl,CXtoken, ip);
                                                                        }
                                                                        if (window.AndroidBridge.closeWebview) {
                                                                                            window.AndroidBridge.closeWebview();
                                                                                        }
                                                                        return;}
                                                                        return setTimeout(CXkeepalive,5000);
                                                           }
                                                       setTimeout(CXkeepalive,20000);
        }
...
let javaAudioPort = null;
window.addEventListener('message', (event) => {
    if (event.data === 'INIT_AUDIO_PIPE' && event.ports && event.ports[0]) {
        javaAudioPort = event.ports[0];
        BxLogger.info("Audio", "Binary Audio Pipe established with Java.");
    }
});

function patchAudio() {
    const OriginalAudioContext = window.AudioContext || window.webkitAudioContext;
    if (!OriginalAudioContext || OriginalAudioContext.__isPatched) return;

    const shadowNodes = new WeakMap();
    const pendingConnections = new WeakMap();
    const originalConnect = AudioNode.prototype.connect;
    const INTERNAL = Symbol('internal');

    // 1. Define Intelligent Capture Worklet (Direct Streaming to Java)
    const workletCode = `
        class CaptureProcessor extends AudioWorkletProcessor {
            constructor() {
                ...
                this.port.onmessage = (e) => {
                    if (e.data.type === 'HANDOVER_JAVA_PORT') {
                        this.javaPort = e.data.port;
                    }
                };
                this.bufferSize = 256;
                this.buffer = new Int16Array(this.bufferSize * 2);
                this.offset = 0;
            }

            process(inputs) {
                const input = inputs[0];
                // Continuous streaming: generate zeros if no input to keep Java clock ticking
                const left = (input && input[0]) ? input[0] : new Float32Array(128);
                const right = (input && input[1]) ? input[1] : left;

                for (let i = 0; i < left.length; i++) {
                    let sL = left[i], sR = right[i];
                    this.buffer[this.offset * 2] = sL < 0 ? sL * 32768 : sL * 32767;
                    this.buffer[this.offset * 2 + 1] = sR < 0 ? sR * 32768 : sR * 32767;
                    this.offset++;

                    if (this.offset >= this.bufferSize) {
                        const chunk = this.buffer.slice();
                        if (this.javaPort) {
                            // DIRECT PIPE: Worklet -> Java
                            this.javaPort.postMessage(chunk.buffer, [chunk.buffer]);
                        } else {
                            // FALLBACK: Worklet -> Main -> Java (Slow)
        ...
        registerProcessor('native-capture-processor', CaptureProcessor);
    `;

    // 2. Non-Recursive Connection Interceptor
    AudioNode.prototype.connect = function(dest, ...rest) {
        if (dest === this.context.destination && !this[INTERNAL]) {
            const shadow = shadowNodes.get(this.context);
            if (shadow) {
                if (this !== shadow) return originalConnect.call(this, shadow, ...rest);
            } else {
                let list = pendingConnections.get(this.context);
                if (!list) {
                    list = [];
                    pendingConnections.set(this.context, list);
                }
                list.push({ source: this, args: rest });
                return dest;
            }
        }
        return originalConnect.call(this, dest, ...rest);
    };
    AudioNode.prototype.connect.toString = () => "function connect() { [native code] }";

    // 3. Transparent Context Proxy
    const ContextProxy = new Proxy(OriginalAudioContext, {
        construct(target, args) {
            const context = new target(...args);
            if (window.AndroidBridge && window.AndroidBridge.setAudioSampleRate) {
                window.AndroidBridge.setAudioSampleRate(context.sampleRate);
            }
            ...
            context.audioWorklet.addModule(url).then(() => {
                const shadow = new AudioWorkletNode(context, 'native-capture-processor');
                shadow[INTERNAL] = true;

                // Hand over the Java Port to the background Worklet thread
                if (javaAudioPort) {
                    shadow.port.postMessage({ type: 'HANDOVER_JAVA_PORT', port: javaAudioPort }, [javaAudioPort]);
                    javaAudioPort = null;
                }

                shadow.port.onmessage = (e) => {
                    // Fallback path if direct pipe wasn't established
                    if (window.AndroidBridge && window.AndroidBridge.sendAudioFrame) {
                        window.AndroidBridge.sendAudioFrame(new Uint8Array(e.data));
                    }
                };

                const silence = context.createGain();
                silence[INTERNAL] = true;
                silence.gain.value = 0;
                originalConnect.call(shadow, silence);
                originalConnect.call(silence, context.destination);

                shadowNodes.set(context, shadow);

                const pending = pendingConnections.get(context);
                if (pending) {
                    pending.forEach(conn => originalConnect.call(conn.source, shadow, ...conn.args));
                    pendingConnections.delete(context);
                }
                BxLogger.info("Audio", "Direct Binary Pipe established (Worklet -> Java)");
            }).catch(e => BxLogger.error("Audio", "Shadow failure", e));
    ...
    Object.defineProperty(ContextProxy, '__isPatched', { value: true, enumerable: false });
    const nativeString = () => "function AudioContext() { [native code] }";
    ContextProxy.toString = nativeString;
    OriginalAudioContext.prototype.constructor.toString = nativeString;

    window.AudioContext = window.webkitAudioContext = ContextProxy;
    BxLogger.info("Audio", "High-Performance Stealth Audio Hijack initialized.");
}
